...
const Navbar = () => {
  ...
  const handleCloseSideBar = (e) => {
    if (
      ...
      !sidebarContentRef.current.contains(e.target)
    ) {
      dispatch(setNavSidebar(false));
    }
  };
  ...
  return (
    <nav
      className={`${styles.navbar} ${navSidebar && styles.active}`}
      ref={sidebarRef}
    >
      <Container>
        <ul
          className={`${styles.navbar__list} ${navSidebar && styles.active}`}
          ref={sidebarContentRef}
        >
          <li className={styles.dropdown}>
            <button
              className={`${styles.dropdown__toggle} ${
                isDropdown && styles.dropdown__toggle__active
              }`}
            ...
            <ul
              className={`${styles.dropdown__menu} ${
                isDropdown && styles.dropdown__menu__active
              }`}
            >
              {list &&
                list?.map((item, index) => (
                  <li key={index} onClick={() => handleNavigate(item.slug)}>
                    {item.slug}
                  </li>
                ))}
            </ul>
          ...
          {menu.map((item, index) => (
            <li key={index}>
              <Link
                to={`/${item.link}`}
                onClick={() => dispatch(setNavSidebar(false))}
              >
                {item.name}
              </Link>
            </li>
