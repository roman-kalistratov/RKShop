      ...
      <div className={styles.store}>
        <div className={styles.mobileTop}>
          <button
            ...
            onClick={() => setIsFilterSidebar(!isFilerSidebar)}
          >
            Filter
          </button>
        ...
        <aside
          className={`${styles.sidebar} ${
            isFilerSidebar ? styles.sidebarActive : ""
          }`}
        >
          <Accordion title="Price" active={true}>
            <div className={styles.filters}>
              ...
              <button
                className="button additional small"
                onClick={() =>
            ...
            <ul className={styles.list}>
              {brands.map((brand) => (
                <li
                  key={brand.id}
                  className={`${styles.listItem} ${
                    crntBrand === brand.brand && styles.active
                  }`}
                  onClick={() => setCrntBrand(brand.brand)}
                >
                  {brand.brand}
                </li>
              ))}
            ...
            <div className={styles.productsTop}>
              {queryCategory && (
                <h3>
                  ...
                  <span onClick={() => dispatch(setQueryCategory(""))}>x</span>
                </h3>
              )}
              {crntBrand && (
                <h3>
                  Brand: {crntBrand}{" "}
                  <span onClick={() => setCrntBrand("")}>x</span>
                </h3>
              )}
              {filteredPrice && (
                <h3>
                  Price: ${filteredPrice.min} - ${filteredPrice.max}{" "}
                  <span onClick={() => setFilteredPrice(undefined)}>x</span>
                </h3>
              )}
